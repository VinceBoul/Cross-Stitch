...
let boatDraw = getCrossStitchObjects();

app.stage.addChild(stage);

let currentCSIndex = 0;
let currentCS = boatDraw.pop();
...
function tickerDraw(drawing){
    if (currentCSIndex < drawing.length){
        currentCS.draw();

        if (currentCS.isFinished()){
            currentCSIndex += 1;
            currentCS = drawing.pop();
        }
...
function CrossStitch(){
    ...
    this.begin = function(posX, posY, color){
        ...
        this.realPath2.lineStyle(lineSize, PIXI.utils.string2hex(color), 1);

        this.realPath1.x = posX;
        this.realPath1.y = posY;
        this.realPath1.lineStyle(lineSize, PIXI.utils.string2hex(color), 1);

        this.container.addChild(this.realPath2);
        this.container.addChild(this.realPath1);
...
function getCrossStitchObjects(){
    let pointsDrawing = [];
    let currentX = 0;
    let crossStitch;
    let currentDrawing =  getBoatDrawing().sort(sortByName).reverse();
    for (let i in currentDrawing){
